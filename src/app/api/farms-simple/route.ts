...
async function executeSqlOnD1(sql: string, params: any[] = []) {
  ...
  const DATABASE_ID = 'e5ef7989-a88e-422c-9f6b-91d2d3adda12';
...
export async function GET() {
  try {
    const data = await executeSqlOnD1('SELECT id, name, description, location, contact, created_at FROM farms ORDER BY id ASC');
...
export async function POST(request: Request) {
  try {
    ...
    const data = await executeSqlOnD1(
      ...
      [name, description || 'Farm CALITEK', location || 'Non spécifié', contact || 'contact@oglegacy.com']
    );
